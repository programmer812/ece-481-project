...
class FlapperSim:
    def __init__(self, backend_server_ip=None, robot_pose=None):
        ...
        self.DT = self.SAMPLING_TIME_INTERVAL / 1000.0
        ...
        if robot_pose is not None:
            ...
            self.x[0:3] = np.array(
                [
                    self.POS_BOUNDS[0]
                    + (self.POS_BOUNDS[1] - self.POS_BOUNDS[0]) * random.random(),
                    self.POS_BOUNDS[2]
                    + (self.POS_BOUNDS[3] - self.POS_BOUNDS[2]) * random.random(),
                    self.POS_BOUNDS[4]
                    + (self.POS_BOUNDS[5] - self.POS_BOUNDS[4]) * random.random(),
                ]
            ).reshape((3, 1))
            self.yaw = 2 * math.pi * random.random()
        self.u = np.zeros((3, 1))  # input: jerk
        self.y = self.x[0:3] + np.random.normal(
            np.zeros((3, 1)), self.NOISE_STD_DEV * np.ones((3, 1))
        )

        # variables for plotting
        self.robot_body_0 = np.array(
            [
                [0.0, 0.0, 0.2 * self.ROBOT_H],
                [0.0, 0.0, self.ROBOT_H],
                [0.0, 0.0, 0.5 * self.ROBOT_H],
                [0.0, 0.0, 0.75 * self.ROBOT_H],
                [
                    self.ROBOT_W / 2.0 * math.sin(math.pi / 12.0),
                    self.ROBOT_W / 2.0 * math.cos(math.pi / 12.0),
                    0.75 * self.ROBOT_H,
                ],
                [
                    -self.ROBOT_W / 2.0 * math.sin(math.pi / 12.0),
                ...
                [self.ROBOT_W / 4.0, self.ROBOT_W / 4.0, 0.0],
                [self.ROBOT_W / 4.0, -self.ROBOT_W / 4.0, 0.0],
                [-self.ROBOT_W / 4.0, self.ROBOT_W / 4.0, 0.0],
                [-self.ROBOT_W / 4.0, -self.ROBOT_W / 4.0, 0.0],
            ]
    ...
    def rotate_and_translate_body(self):
        ...
        t[2] -= self.ROBOT_H
        self.robot_body = R @ self.robot_body_0 + np.tile(
            t, (1, self.robot_body_0.shape[1])
        )
    ...
    def __init_plot(self):
        ...
        self.figure.set_size_inches(
            1200 / plt.rcParams["figure.dpi"], 1200 / plt.rcParams["figure.dpi"]
        )
        self.axes.plot(
            [self.POS_BOUNDS[i] for i in [0, 0, 1, 1, 0]],
            [self.POS_BOUNDS[i] for i in [2, 3, 3, 2, 2]],
            [self.POS_BOUNDS[i] for i in [4, 4, 4, 4, 4]],
            "k",
        )
        self.axes.plot(
            [self.POS_BOUNDS[i] for i in [0, 0, 1, 1, 0]],
            [self.POS_BOUNDS[i] for i in [2, 3, 3, 2, 2]],
            [self.POS_BOUNDS[i] for i in [5, 5, 5, 5, 5]],
            "k",
        )
        self.axes.plot(
            [self.POS_BOUNDS[i] for i in [0, 0]],
            [self.POS_BOUNDS[i] for i in [2, 2]],
            [self.POS_BOUNDS[i] for i in [4, 5]],
            "k",
        )
        self.axes.plot(
            [self.POS_BOUNDS[i] for i in [0, 0]],
            [self.POS_BOUNDS[i] for i in [3, 3]],
            [self.POS_BOUNDS[i] for i in [4, 5]],
            "k",
        )
        self.axes.plot(
            [self.POS_BOUNDS[i] for i in [1, 1]],
            [self.POS_BOUNDS[i] for i in [3, 3]],
            [self.POS_BOUNDS[i] for i in [4, 5]],
            "k",
        )
        ...
        for s in self.robot_body_segments:
            ...
            self.handles.append(
                self.axes.plot(
                    [self.robot_body[0, k] for k in [i, j]],
                    [self.robot_body[1, k] for k in [i, j]],
                    [self.robot_body[2, k] for k in [i, j]],
                    "k",
                )[0]
            )
        ...
        self.axes.set_zlim(0.0, self.POS_BOUNDS[5])
        self.axes.view_init(elev=15.0, azim=-135.0, roll=0.0)
    ...
    def get_output_measurement(self):
        delta_time_get_output = (
            int(round(time.time() * 1000)) - self.last_time_get_output
        )
    ...
    def step(self, x, u):
        ...
        self.x[0] = (
            self.x[0]
            + self.x[3] * self.DT
            + self.x[6] * self.DT**2 / 2
            + self.u[0] * self.DT**3 / 6
        )
        self.x[1] = (
            self.x[1]
            + self.x[4] * self.DT
            + self.x[7] * self.DT**2 / 2
            + self.u[1] * self.DT**3 / 6
        )
        self.x[2] = (
            self.x[2]
            + self.x[5] * self.DT
            + self.x[8] * self.DT**2 / 2
            + self.u[2] * self.DT**3 / 6
        )
